...
class Game(object):
    ...
    def first_turn(self, start_ind):
        ''' Removes piece in place indexed by \start_index. Does not check whether the turn being played is the first. '''
        
        self.board[start_ind[0]][start_ind[1]] = FREE
        self.num_pieces -= 1
        self.history.append(start_ind)
    ...
    def play(self):
        ...
        print('\nHistory:\n')
        item = self.history[0]
        print(' - Removed ' + '  abcdefg'[item[0]] + str(item[1]-1))
        for item in self.history[1:]:
            print(' - Moved '+ '  abcdefg'[item[0][0]] + str(item[0][1]-1) + ' ' + {UP: 'up', DOWN: 'down', LEFT: 'left', RIGHT: 'right'}[item[1]])
        print()
