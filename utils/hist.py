...
def hist(lst, num_bins = -1, title = '', allnames = False, max_height = 20):
    ...
    max_lab = str(int(min(bins) + (max(bins) - min(bins))*(2*max_height - 1)/(2 * max_height)))
    spaces_till_y = len(max_lab)
    ...
    # Two spaces for each bin plus four at the end
    xaxis += '-'*(2 * num_bins + 4)
    xlabs += ' '*(2 * num_bins + 4)
    ...
    for h in range(max_height):
        ...
        # y-label
        ymax = min(bins) + (max(bins) - min(bins))/max_height*(h+1)
        ymin = min(bins) + (max(bins) - min(bins))/max_height*h
